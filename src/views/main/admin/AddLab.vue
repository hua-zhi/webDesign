<template>
  ...
  >
    ...
    <el-form-item label="实验室编号" prop="number">
      <el-input v-model="newlab.number"></el-input>
    </el-form-item>
    <el-form-item label="机器数量" prop="deviceNum">
...
import { UPDATE_LAB } from "@/store/VuexTypes";
export default {
  setup() {
    const store: Store<State> = useStore();
    const lab = computed(() => store.state.labList);
    ...
    const createLab = () => {
      store.dispatch(UPDATE_LAB, {
        id: newlab.value.id,
        place: newlab.value.number,
        deviceNum: newlab.value.machine,
